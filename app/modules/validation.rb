module Validation
  ...
  def validate_number_of_player(players)
    number_players = [2,4,8,16,32]
    render json: { error: 'invalid number of player' } unless number_players.include?(players.size)
    true
  end
  ...
  def validate_each_param(parameters, keys)
    parameters.each do |key, value|
      render json: {error: "invalid parameter"}, status: :bad_request unless keys.include?(key)
    end
  end
end