class TournamentsController < ApplicationController
  ...
  def show
    render json: Tournament.where(clan_tag: params[:id].to_s)
  end
  ...
  def create
    ...
    if tournament.save
      ...
      render json: { error: "the tournament can't be created" }, 
             status: :unprocessable_entity
    end
  end

  def update
    tournament = Tournament.where(clan_tag: params[:id].to_s).last
    if tournament.update(tournaments_params_update)
      render json: tournament, status: :ok
    else
      render json: { error: "the tournament can't be edited" },
             status: :unprocessable_entity
    end
  end
  ...
  def tournaments_params_create
    params.require(:tournament).permit(:clan_tag, :time_per_round, :players,
                                       :rounds)
  end

  def tournaments_params_update
    params.require(:tournament).permit(:first_place, :second_place, :best_streak,
                                       :unbeaten)
  end
end
